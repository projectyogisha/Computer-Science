// cuda Matrix Addition using A[i][j] and (*a)[j] declaration and 2 dimension blocks x and y. 
...
{
    int i = threadIdx.x;	 // columns
	int j = threadIdx.y;     // rows
    C[i][j] = A[i][j] + B[i][j];
}
